...
_new_instance(gchar *filename, GlobalConfig *cfg)
{
...
_wildcard_new_instance(gchar *filename, GlobalConfig *cfg)
{
  AFFileWildcardSourceDriver *self = g_new0(AFFileWildcardSourceDriver, 1);
  
  log_src_driver_init_instance(&self->super.super, cfg);
...
static void
_init_instance(AFFileSourceDriver *self, gchar *filename, GlobalConfig *cfg)
{
...
{
  AFFileSourceDriver *self = _new_instance(filename, cfg);

  _init_instance(self, filename, cfg);

  return &self->super.super;
}

LogDriver *
affile_wildcard_sd_new(gchar *filename, GlobalConfig *cfg)
{
  AFFileWildcardSourceDriver *self = _wildcard_new_instance(filename, cfg);

  _init_instance(&self->super, filename, cfg);
