...
_afinet_dd_start_failback_timer(AFInetDestDriver *self)
{
  ...
  elapsed_time = timespec_diff_msec(&now, &(self->failback_timer.expires));
  self->failback_timer.expires = now;
...
static inline void
_afinet_dd_start_failback_timer_with_reset(AFInetDestDriver *self)
{
...
_afinet_dd_tcp_probe_succeded(AFInetDestDriver *self)
{
  ...
  msg_notice("Probing primary server successful",
             evt_tag_int("successful-probes-received", self->successfull_probes_received),
             evt_tag_int("successful-probes-required", self->successfull_probes_required));
  ...
  if (getsockopt(self->failback_fd.fd, SOL_SOCKET, SO_ERROR, &error, &errorlen) == -1)
    {
      ...
      _afinet_dd_start_failback_timer_with_reset(self);
      return;
    }
...
const gchar *
afinet_dd_get_hostname(AFInetDestDriver *self)
{
...
static gboolean
_is_primary(AFInetDestDriver *self)
{
...
static const gchar *
_get_hostname(GList *it)
{
  return (const gchar *)(it->data);
}

static void
_step_current_server_candidate(AFInetDestDriver *self)
{
  GList *current = self->current_server_candidate; //TODO: find a better name
  ...
  if (self->is_failback_mode && _is_primary(current))
    {
      _afinet_dd_start_failback_timer(self);
  ...
  msg_warning("Current server is inaccessible, sending the messages to the next failover server",
            evt_tag_str("current", _get_hostname(current)),
            evt_tag_str("failover", afinet_dd_get_hostname(self)),
            log_pipe_location_tag(&self->super.super.super.super));
}
...
afinet_dd_setup_addresses(AFSocketDestDriver *s)
{
  ...
  if (!resolve_hostname_to_sockaddr(&self->super.dest_addr, self->super.transport_mapper->address_family, afinet_dd_get_hostname(self)))
    return FALSE;
...
afinet_dd_get_dest_name(const AFSocketDestDriver *s)
{
  ...
  static const gchar *hostname = afinet_dd_get_hostname(self);
