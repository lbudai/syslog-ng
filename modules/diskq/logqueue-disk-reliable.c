...
_move_message_from_qbacklog_to_qreliable(LogQueueDiskReliable *self)
{
  ...
  stats_counter_add(self->super.super.counters.memory_usage, log_msg_get_size(ptr_msg));
}
...
_rewind_backlog(LogQueueDisk *s, guint rewind_count)
{
  ...
  stats_counter_add (self->super.super.counters.queued_messages, rewind_count);
}
