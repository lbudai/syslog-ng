...
{
  state->gi = g_new0(MMDB_s, 1);

  int mmdb_status = MMDB_open(state->database, MMDB_MODE_MMAP, state->gi);
                           ...
                           gint argc, gchar *argv[], GError **error)
{
  ...
  {
    { "database", 'd', 0, G_OPTION_ARG_FILENAME, &state->database, "mmdb database location", NULL },
...
tf_geoip_maxminddb_call(LogTemplateFunction *self, gpointer s, const LogTemplateInvokeArgs *args, GString *result)
{
  ...
  MMDB_lookup_result_s mmdb_result =
    MMDB_lookup_string(state->gi, argv[0]->str, &gai_error, &mmdb_error);

  if (!mmdb_result.found_entry)
    {
      mmdb_problem_to_debug(gai_error, mmdb_error, "tflookup");
      return;
    }
  ...
  if (mmdb_error != MMDB_SUCCESS)
    {
      mmdb_problem_to_debug(0, mmdb_error, "tfget_value");
      return;
    }
