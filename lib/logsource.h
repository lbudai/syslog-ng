...
struct _LogSource
{
  ...
  void (*schedule_dynamic_window_redistribute)(LogSource *s);
};
...
static inline void
log_source_schedule_dynamic_window_redistribute(LogSource *s)
{
  if (!s || !s->schedule_dynamic_window_redistribute)
    return;
  s->schedule_dynamic_window_redistribute(s);
}
