...
log_queue_fifo_push_tail(LogQueue *s, LogMessage *msg, const LogPathOptions *path_options)
{
  ...
  if (/*log_queue_fifo_get_length(s) < self->qoverflow_size*/ !log_msg_source_reached_memory_limit(msg))
    {
      node = log_msg_alloc_queue_node(msg, path_options);
  ...
  else
    {
      ...
      if (path_options->flow_control_requested)
        ...
        {
          msg_debug("Source of msg: memory limit reached, dropping message",
          log_pipe_location_tag(log_msg_get_source(msg)),
