...
{
  gboolean suspended;
  //TODO: adjust_window_to_be_reclaimed... //better name?
      ...
      if (old > 0)
        {
          ...
          else // diff will be the real window_size_increment
            {
...
_decrease_window(LogSource *self, double dec_factor)
{
  ...
  double dec_win_size = self->full_window_size - (double)self->full_window_size * dec_factor;
  gsize new_full_window_size = MAX(self->options->init_window_size, (gsize)dec_win_size);
...
log_source_dynamic_window_realloc(LogSource *self)
{
  ...
  if (self->dynamic_window.last_ack_rate_avg == 0)
    {
      self->dynamic_window.last_ack_rate_avg = current_ack_rate_avg == 0 ? 1: 0;
      return; //TODO:check
    }
...
//  gboolean is_dest_performance_decreased = change_factor < 1.0 * (self->options->dynamic_window_decrease_threshold / 100.0);
  ...
  gboolean have_to_decrease = /*window_almost_full && */is_dest_performance_decreased;
  gboolean have_to_increase = /*window_almost_empty &&*/ is_dest_performance_increased;
  ...
  if (have_to_increase)
    {
//      _increase_window(self, MAX(MIN(fabs(1.0 - change_factor), 2.0), 0.1));
  ...
  msg_warning("DYNWINSTAT_PER_CONN",
              ...
              evt_tag_printf("performance", "%s", is_dest_performance_increased ? "increased" : is_dest_performance_decreased ?
                             "decreased" : "unchanged"),
              evt_tag_printf("increase_window", "%s", have_to_increase ? "yes" : "no"),
              evt_tag_printf("decrese_window", "%s", have_to_decrease ? "yes" : "no"),
              evt_tag_printf("source", "%p", self),
