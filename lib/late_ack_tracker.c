...
late_ack_tracker_manage_msg_ack(AckTracker *s, LogMessage *msg, AckType ack_type)
{
  ...
  late_ack_tracker_lock(s);
  {
    ...
    g_assert(self->ack_record_storage_tail == g_list_last(self->ack_record_storage)); //TODO: remove
    gint idx = g_list_index(self->ack_record_storage, msg->ack_record);
...
late_ack_tracker_init_instance(LateAckTracker *self, LogSource *source)
{
  ...
  self->ack_record_storage_tail = NULL;
